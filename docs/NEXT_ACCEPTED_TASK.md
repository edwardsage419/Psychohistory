...
Generate a deterministic recovery-target manifest over the complete frozen 120-case `gkg-semantics-v2` sample, then prepare the next bounded evidence-recovery batch without changing frozen sample membership, the Phase 6A document-identity contract, human identity judgments, or semantic conclusions.
...
12. `studies/gkg-semantics-v2/context-sufficiency-correction.json`
13. `studies/gkg-semantics-v2/human-identity-review-results.csv`
14. `docs/PHASE6A1_CONTEXT_SUFFICIENCY_CORRECTION.md`
15. `scripts/gkg_recovery.py`
16. `scripts/phase6a1_recovery.py`

Implementation may inspect `scripts/gkg_semantics.py` for existing canonical serialization, digest, authentication, and validation helpers.

The historical `phase6a1-availability.json` and `docs/PHASE6A1_CONFIRMED_COVERAGE_AUDIT.md` may be read only as provenance for the pre-correction state. Their old E3-only coverage counts are superseded for current planning by the correction artifacts above.
...
2. Read raw bytes for `evidence.json` and `phase6a1-triage.json`.
3. Verify the byte SHA-256 of those files against the independently accepted artifact hashes in `assessment-manifest.json`.
4. Parse the authenticated bytes.
5. Compute canonical object digests with the accepted `gkg_semantics.digest()` helper.
6. Call/reuse `phase6a1_recovery.resolve()` with those canonical object digests as `base_root` and `delta_root`.
7. Recompute evidence sufficiency with current version `1.0.1` and verify the result against `context-sufficiency-correction.json`.
8. Verify the human identity CSV against accepted Git object `6ea6884e73918f7f3d3a52c076885dc409f5aca7` before joining by `case_id`.
9. Keep machine evidence, human identity review, and generated manifest artifacts separate.

A digest computed from an unauthenticated candidate file is not an external trust root. Once the file bytes have been independently authenticated, computing the canonical object digest required by `resolve()` is valid transformation of authenticated input.
...
8. Otherwise, an `identity_unresolved` case may receive Tier B only when accepted resolved evidence contains an explicit URI-equivalent `recovered_url` or canonical URL under existing `gkg_recovery.uri()` semantics.
...
5. accepted retrieval-method hierarchy when the method requires ordering
6. frozen sample order

If a current non-review-ready method cannot be mapped without a new scientific rule, stop and escalate rather than inventing a rank.
...
Top-level bindings must identify the frozen sample, authenticated machine inputs, sufficiency correction contract, current implementation identity, and human review artifact.
...
The manifest may list only protocol-permitted method families:
...
`unavailable` is a state, not a method.
...
* frozen evidence, delta, protocol, sample, and human-review artifacts remain byte unchanged

Add mutation tests for duplicate cases, frozen metadata changes, trust-root mismatches, human-to-E2/E3 promotion, tier-precedence errors, exclusion removal, review-ready/E1 confusion, and first-batch drift.
...
Only after explicit authorization, run bounded objective evidence/context recovery against the accepted manifest, recompute E2/E3 review-ready coverage mechanically, and repeat while preserving the frozen sample and protocol.
