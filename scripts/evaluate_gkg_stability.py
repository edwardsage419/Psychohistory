...
def evaluate(batches, target_days=14, minimum_days=7):
    topics = sorted({topic for b in batches for topic in b.get("topics", {})})
    topic_report = {}
    for topic in topics:
        rates = [b["topics"][topic]["all_rate_per_1000_documents"] for b in batches if topic in b.get("topics", {})]
        primary = [b["topics"][topic]["primary_rate_per_1000_documents"] for b in batches if topic in b.get("topics", {})]
        secondary = [b["topics"][topic]["secondary_rate_per_1000_documents"] for b in batches if topic in b.get("topics", {})]
        if rates:
            m = mean(rates)
            sd = stdev(rates)
    ...
    valid_batches = [b for b in batches if b.get("dataset", {}).get("rows", 0) > 0 and b.get("dataset", {}).get("bad_rows", 1) == 0]
    unique_sources = len({b.get("source", {}).get("gkg") for b in valid_batches})
    enough = len(valid_batches) >= minimum_days
    target_reached = len(valid_batches) >= target_days
